...
There is no automated test; verification requires launching Skyrim VR with the DLL installed. The plugin logs to `Documents\My Games\Skyrim VR\SKSE\TakeAllLockpicksVR.log`. Diagnostic logging in the hook prints every `RemoveItem` involving a lockpick or a transfer to the player (item, count, reason, extraList presence, source/destination FormIDs) — this is the primary debugging instrument. When testing changes, exercise: corpse loot (multi and single pick), chest loot, follower take, merchant purchase (must stay exact), giving picks to a follower (must stay exact), and the quantity prompt (choice must be respected).
